...
plot_long_tbl <- function(long_tbl, title, colorCol) {
  ...
  ggplot(long_tbl, aes(metric, y = AUC)) + 
...
plot_cancer <- function(tbl, cancer_type) {
  ...
  ggsave(glue("aucs/plots/{cancer_type} plot.png"))
}
...
overall_plot <- function(tibbles) {
  ...
  ggsave(glue("aucs/plots/combined plot CellLine.png"))
  plot_long_tbl(combined, "All cancers by type", "CancerType")
  ggsave(glue("aucs/plots/combined plot CancerType.png"))
}
