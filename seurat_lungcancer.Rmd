...
mi.graph <- build_dist_matrix(cells = comb.cells, counts = counts, metric = function(x,y) 1 - NMI(x,y)) %>%
    as_tbl_graph(directed = FALSE)
cor.graph <- build_dist_matrix(cells = comb.cells, counts = counts, metric = function(x,y) 1 - abs(cor(x,y))) %>%
                as_tbl_graph(directed = FALSE)
```
...
```{r create plots}
create_clusters("Pts 3 and 4 combined exp minmax + 1", mi.graph = mi.graph, cor.graph = cor.graph) 
```
