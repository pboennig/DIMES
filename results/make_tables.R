...
make_pca_tibbles <- function(cell_lines) {
  tbl <- read_csv(glue("aucs/pca/{cell_lines[1]}_PCA.csv"), col_types = "_cd")
  colnames(tbl)[2]  <- cell_lines[1]
  if (length(cell_lines) > 1) {
    for (i in 2:length(cell_lines)) {
      tbl[, cell_lines[i]] <- read.csv(glue("aucs/pca/{cell_lines[i]}_PCA.csv", col_types="_cd"))$auc
    }
  }
  tbl
}
...
mean_cancer_type <- function(tibbles, numCellLines, pca=F) {
  dodge_w <- .5
  long_tbls <- lapply(tibbles, pivot_longer, cols=-metric, names_to="CellLine", values_to="AUC")
  for (i in 1:length(long_tbls)) {
    long_tbls[[i]]$CancerType <- rep(names(long_tbls)[i], nrow(long_tbls[[i]]))
  ...
  ggplot(combined, aes(x=metric, y = mean_AUC, color = CancerType)) + 
    ...
    theme(axis.text.x = element_text(angle=45, vjust=1, hjust=1))
  if (pca) {
    ggsave(glue("plots/pca/combined plot mean CancerType.png"), width = 10, height = 6)
  } else {
...
print(sum(numCellLines))
tibbles <- lapply(cellLineMap, make_tibbles)
pca_tibbles <- lapply(cellLineMap, make_pca_tibbles)
...
mean_cancer_type(pca_tibbles, numCellLines, pca=T)
